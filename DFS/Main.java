...
public class Main {
     ...
     */
    public static void DFS1(int[] box, int[] used, int idx) {
    ...
    public static int DFS2(Map<Integer,Employee> info, int id){
    ...
    public static void DFS3(int[][] image,int row,int col,int[][] visited,int newx,int newy,int oldC,int newC){
        ...
        for (int i=0;i<4;i++){
            ...
            if (image[nextx][nexty] == oldC && visited[nextx][nexty] == 0) {
                //继续往下搜索
                DFS3(image,row,col,visited,nextx,nexty,oldC,newC);
    ...
    public static int[][] floodFill(int[][] image,int sr,int sc,int newColor){
        ...
        DFS3(image,row,col,visited,sr,sc,oldC,newColor);
        return image;
    ...
    public static void DFS4(char[][] board,int row,int col,int x,int y){
    ...
    public static void solve(char[][] board){
        ...
        for (int i=0;i<col;i++){
            //第一行
            if (board[0][i]=='o'){
                DFS4(board,row,col,0,i);
            }


            //最后一行
            if (board[row-1][i]=='o'){
                DFS4(board,row,col,row-1,i);
        ...
        for (int i=1;i<row-1;i++){
            //第一列
            if (board[i][0]=='o'){
                DFS4(board,row,col,i,0);

            }

            //最后一列
            if (board[i][col-1]=='o'){
                DFS4(board,row,col,i,col-1);
    ...
    public static void DFS5(char[][] grid,int row,int col,boolean[][] visited,int x,int y){
        //1.标记
        visited[x][y] = true;
        ...
        for (int i=0;i<4;i++){
            ...
            if (grid[nx][ny]=='1'&&!visited[nx][ny]){
                ...
                DFS5(grid,row,col,visited,nx,ny);
            }
    ...
    public static void DFS6(int[] candidates,int target,List<List<Integer>> allRet,
        ...
        for (int i=prePos;i<candidates.length;i++){
            ...
            DFS6(candidates,target,allRet,curRet,curSum+candidates[i],i);
    ...
    public static List<List<Integer>> combinationSum(int[] candidates,int target){
        ...
        DFS6(candidates,target,allRet,curRet,0,0);
        return allRet;
    }
    ...
    public static void DFS7(String titles,int[] visit,String curStr,Set<String> allRet){
        ...
        for (int i=0;i<titles.length();i++){
            ...
            if (visit[i] == 0) {
                visit[i] = 1;
                DFS7(titles,visit,curStr+titles.charAt(i),allRet);
                //回溯
                visit[i] = 0;
            }
        }
    ...
    public List<List<String>> solveNQueens(int n) {
        ...
        int[] setQueen = new int[n];

        //从第0行开始放
        enQueen(setQueen,0,n);
        return res;
    ...
    private void enQueen(int[] setQueen,int row, int n) {

        //直到row==n，说明已经全部摆放完毕
        if (row == n) {
            //打印出来
            showQueen(setQueen,n);
            return;
        ...
        for (int col =0;col<n;col++){
            ...
            //确定这个位置是否可以放置皇后
            if (realQueen(setQueen,row,n,col)){
                setQueen[row] = col;
                enQueen(setQueen,row+1,n);
    ...
    private boolean realQueen(int[] setQueen,int row,int n,int col) {
        int left = col-1;
        int right = col+1;
        ...
        for (int i=row-1;i>=0;i--) {
            ...
            //左上方有不能放
            if (left>=0&&setQueen[i]==left)return false;
            //右上方有也不能放
            if (right<=n&&setQueen[i]==right)return false;
            left++;
            right--;
    ...
    private void showQueen(int[] setQueen, int n) {
